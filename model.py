...
class get_data:
    @staticmethod
    def load_weapon_index(weapon):
        try:
            with open("weapons_data.json", "r", encoding="utf-8") as file:
                data = json.load(file)
                index_data = data[weapon]["index_table"]
            return index_data
        except FileNotFoundError:
    ...
    @staticmethod
    def load_weapon_camo(weapon, camo):
        try:
            with open("weapons_data.json", "r", encoding="utf-8") as file:
                data = json.load(file)
                camo_data = data[weapon][camo]
            return camo_data
        except FileNotFoundError:
            showerror("Error", "Carpeta JSON no exite cierra el mensaje para crearlo")
            set_data.create_json_file()
...
class set_data:
    ...
    def create_json_file(): 
        if not os.path.exists("weapons_data.json"):
            headings = ["Militar", "Especial", "Oro", "Rescate del Rey", "Catalizador"]
            weapons_data = {weapon: {camo: "" for camo in headings} | {"index_table": [index for index in range(0, 6)]} for weapon in ready_data_for_table.weapons}
            with open("weapons_data.json", "w", encoding="utf-8") as file:
                json.dump(weapons_data, file, indent=4, ensure_ascii=False)

    @staticmethod
    def add_camo_to_json(weapon, camo):
        try:
            with open("weapons_data.json", "r", encoding="utf-8") as file:
                data = json.load(file)
                data[weapon][camo] = "✔️"
    ...
    @staticmethod
    def del_camo_from_json(weapon, camo, index):
        try:
            with open("weapons_data.json", "r", encoding="utf-8") as file:
                data = json.load(file)
                data[weapon][camo] = ""
